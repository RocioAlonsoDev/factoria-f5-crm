                ...
                <table className="items-center w-full bg-transparent dark:border-neutral-200">
                        ...
                        <tr>
                            ...
                            {tittlesCompetence.map((tittle, index) => (
                                <th 
                                    ...
                                    key={index}
                                    colSpan={contents[0].competences[tittle].length}  // Aquí calculamos el colspan
                                >
                                    {tittle}
                                </th>
                            ))}
                    ...
                    <tbody className="border-b font-medium dark:border-neutral-200">
                        {contents && contents.map((content, index) => (
                            <tr key={index}>
                                <td className="border px-6 py-4 dark:border-neutral-200">{content.fecha}</td>
                                <td className="border px-6 py-4 dark:border-neutral-200">{content.tipo}</td>
                                {tittlesCompetence.flatMap((tittle) => (
                                    content.competences[tittle].map((value, i) => (
                                        <td key={i} className="border-r px-6 py-4 dark:border-neutral-200">
                                            {value}
                                        </td>
                                    ))
                                ))}
