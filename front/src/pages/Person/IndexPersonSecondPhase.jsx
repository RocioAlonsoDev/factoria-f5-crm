...
import FilterButton from '../../components/atoms/FilterButton'; // Cambié el nombre del componente del botón
...
export default function IndexPersonSecondPhase() {
  ...
  const applyFilter = () => {
    if (selectedFilterStatus === '') {
      setPeople(originalPeople); // Restaura todos los registros originales
    } else {
      const filteredData = originalPeople.filter((person) => {
        const exercicesRequirementId = 6;
        const exercicesValue = parseInt(selectStatus[`${person.id}_${exercicesRequirementId}`]) || 0;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setOriginalPeople(response.data.data); // Establece la copia de seguridad
  ...
  const data = people.map((person) => {
    ...
    const exercicesValue = selectStatus[`${person.id}_${exercicesRequirementId}`] || '';
      ...
      'Estado de ejercicios': (
        <div className="flex items-center">
          ...
          <select
            ...
            onChange={(e) =>
              handleSelectChange(person.id, exercicesRequirementId, e.target.value)
            }
          >
    ...
    <div className='md:block md:fixed md:top-[107px] md:left-64 md:right-0 w-auto p-2'>
      ...
      <div className={`modal ${isFilterModalOpen ? 'block' : 'hidden'}`}>
        <div className="modal-content bg-white w-1/2 mx-auto my-16 p-6 rounded-lg shadow-lg">
          <span className="close absolute top-4 right-4 cursor-pointer" onClick={closeFilterModal}>&times;</span>
          <h2 className="text-lg font-semibold mb-4">Filtrar por Estado de Ejercicios</h2>
      ...
      <TableAtom tableTitle={'SEGUNDO FORMULARIO: Todas las personas en el proceso'} columns={columns} data={data} />
